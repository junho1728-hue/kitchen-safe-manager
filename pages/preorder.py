...
with tab_new:
    ...
    if restaurants:
        ...
        restaurant_val = "" if selected_rest == "(미분류)" else selected_rest
    else:
        restaurant_val = ""
        ...
        if st.button("📊 AI 등급 분석 후 등록", use_container_width=True, type="primary", key="pre_analyze"):
            if not settings.get("api_key"):
                ...
                if not lines:
                    ...
                    st.session_state["pre_restaurant_val"] = restaurant_val
                    st.rerun()
    ...
    if "pre_analyzed" in st.session_state and st.session_state["pre_analyzed"]:
        ...
        saved_rest = st.session_state.get("pre_restaurant_val", restaurant_val)

        st.markdown("---")
        rest_label = f" | 🏷️ {saved_rest}" if saved_rest else ""
        st.subheader(f"AI 분석 결과 — 입고 예정: {intake_str}{rest_label}")
        ...
        with col_save:
            if st.button("💾 발주표 저장", use_container_width=True, type="primary", key="pre_save"):
                ...
                st.session_state.pop("pre_restaurant_val", None)
                st.rerun()
        with col_cancel:
...
with tab_list:
    ...
    if not preorders:
        ...
        # ── 코너 필터 버튼 ──
        if restaurants:
            all_rest_in_preorders = sorted({p.get("restaurant", "") for p in preorders if p.get("restaurant")})
            if all_rest_in_preorders:
                if "pre_list_restaurant" not in st.session_state:
                    st.session_state["pre_list_restaurant"] = "전체"

                btn_labels = ["전체"] + all_rest_in_preorders
                cols = st.columns(len(btn_labels))
                for col, label in zip(cols, btn_labels):
                    selected = st.session_state["pre_list_restaurant"] == label
                    btn_type = "primary" if selected else "secondary"
                    if col.button(label, key=f"pre_list_rest_{label}", type=btn_type, use_container_width=True):
                        st.session_state["pre_list_restaurant"] = label
                        st.rerun()

                selected_list_rest = st.session_state["pre_list_restaurant"]
                if selected_list_rest != "전체":
                    preorders = [p for p in preorders if p.get("restaurant", "") == selected_list_rest]
        ...
        for d in dates:
            ...
            for p in day_items:
                ...
                rest_badge = f' <span style="color:#aaa; font-size:0.8rem;">🏷️ {p["restaurant"]}</span>' if p.get("restaurant") else ""
        ...
        # ── 완료된 항목 일괄 삭제 ──
        all_preorders = load_preorders()
        completed = [p for p in all_preorders if p["status"] == "completed"]
        if completed:
            st.markdown("---")
            if st.button(f"🗑 완료된 항목 {len(completed)}건 삭제", key="del_completed"):
                save_preorders([p for p in all_preorders if p["status"] != "completed"])
                st.rerun()
