...
1. corner (코너명/매장명):
   - "공급받는자" 또는 "납품처" 쪽의 상호명에서 코너명을 추출하세요.
   - 상호명에 괄호가 있으면 괄호 안의 내용이 코너명입니다.
     예: "푸드_이대서울(불고기명가)" → corner는 "불고기명가"
     예: "푸드_이대서울(청진동순두부)" → corner는 "청진동순두부"
   - 괄호가 없으면 상호명 전체에서 코너/매장/부서를 추출하세요.
...
no_expiry 판단:
- 소비기한이 법적으로 없거나 사실상 반영구적인 식자재만 true로 설정하세요.
- true 대상 (매우 제한적): 설탕, 소금, 쌀, 물, 빙과류(얼음)
- false 대상 (소비기한 있음): 참기름, 식용유, 후추, 간장, 식초, 물엿, 고춧가루, 미림, 맛술, 조미료 등 모든 가공식품
- 확실하지 않으면 반드시 false로 설정하세요.
